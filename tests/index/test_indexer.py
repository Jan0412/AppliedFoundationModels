...
def test_schema_has_expected_fields(indexer):
    ...
    assert names == ["id", "collection_id", "vector", "path", "timestamp"]
...
def test_open_or_create_table_returns_existing(indexer):
    ...
    t1.add([{
        ...
        "vector": [0.0] * 8, "path": "", "timestamp": 0.0,
    }])
...
def test_normalize_paths_derive_sha1_ids(indexer):
    records = indexer._normalize(
        ["/foo.png", "/bar.png"], collection_id="c", ids=None, timestamps=None
    )
...
def test_normalize_paths_ids_differ_per_collection(indexer):
    r1 = indexer._normalize(["/x.png"], collection_id="A", ids=None, timestamps=None)
    r2 = indexer._normalize(["/x.png"], collection_id="B", ids=None, timestamps=None)
    assert r1[0]["id"] != r2[0]["id"]


def test_normalize_paths_ids_are_stable(indexer):
    """Same input → same id on repeat calls."""
    r1 = indexer._normalize(["/p.png"], collection_id="c", ids=None, timestamps=None)
    r2 = indexer._normalize(["/p.png"], collection_id="c", ids=None, timestamps=None)
    assert r1[0]["id"] == r2[0]["id"]
...
def test_normalize_pil_derives_id_from_png_bytes(indexer, fake_pil_image):
    records = indexer._normalize(
        [fake_pil_image], collection_id="c", ids=None, timestamps=None
    )
    buf = io.BytesIO()
...
def test_normalize_pil_explicit_id_overrides(indexer, fake_pil_image):
    records = indexer._normalize(
        [fake_pil_image], collection_id="c", ids=["mine"], timestamps=None
    )
...
def test_normalize_accepts_pathlib_path(indexer):
    records = indexer._normalize(
        [Path("/some/p.png")], collection_id="c", ids=None, timestamps=None
    )
    assert records[0]["path"] == "/some/p.png"


# ---------------------------------------------------------------------------
# _normalize — timestamps
# ---------------------------------------------------------------------------


def test_normalize_default_timestamp_is_zero(indexer):
    r = indexer._normalize(["/a.png"], collection_id="c", ids=None, timestamps=None)
    assert r[0]["ts"] == 0.0


def test_normalize_timestamps_are_stored(indexer):
    r = indexer._normalize(
        ["/a.png", "/b.png"], collection_id="c", ids=None, timestamps=[1.5, 2.5]
    )
    assert r[0]["ts"] == 1.5 and r[1]["ts"] == 2.5
...
def test_normalize_load_callable_loads_pil_from_path(indexer, make_image_files):
    ...
    records = indexer._normalize(paths, collection_id="c", ids=None, timestamps=None)
    img0 = records[0]["load"]()
    img1 = records[1]["load"]()
...
def test_normalize_load_callable_pil_converts_non_rgb(indexer):
    ...
    records = indexer._normalize([grey], collection_id="c", ids=None, timestamps=None)
...
def test_normalize_mismatched_ids_length_raises(indexer):
    with pytest.raises(ValueError, match="len\\(ids\\)"):
        indexer._normalize(
            ["/a.png", "/b.png"], collection_id="c", ids=["only-one"], timestamps=None
        )


def test_normalize_mismatched_timestamps_length_raises(indexer):
    with pytest.raises(ValueError, match="len\\(timestamps\\)"):
        indexer._normalize(
            ["/a.png", "/b.png"], collection_id="c", ids=None, timestamps=[1.0]
        )
...
def test_normalize_unsupported_image_type_raises_typeerror(indexer):
    with pytest.raises(TypeError, match="must be"):
        indexer._normalize([42], collection_id="c", ids=None, timestamps=None)
...
def test_insert_writes_one_row_per_image(indexer, make_image_files):
    ...
    job = indexer.insert(paths, collection_id="c1")
...
def test_insert_returns_jobstatus(indexer, make_image_files):
    ...
    job = indexer.insert(paths, collection_id="c2")
...
def test_insert_uses_supplied_job_id(indexer, make_image_files):
    paths = make_image_files(2)
    job = indexer.insert(paths, collection_id="c3", job_id="my-job-77")
...
def test_insert_invokes_model_embed_images(indexer, mock_siglip_model, make_image_files):
    paths = make_image_files(3)
    indexer.insert(paths, collection_id="c4")
...
def test_insert_batches_inputs_into_batch_size_chunks(indexer, mock_siglip_model, make_image_files):
    ...
    paths = make_image_files(5)
    indexer.insert(paths, collection_id="c5")
...
def test_insert_stores_collection_id_on_every_row(indexer, make_image_files):
    paths = make_image_files(3)
    indexer.insert(paths, collection_id="myroom")
    rows = indexer.db.open_table("myroom").to_arrow().to_pylist()
    assert all(r["collection_id"] == "myroom" for r in rows)


def test_insert_stores_path_and_timestamp(indexer, make_image_files):
    paths = make_image_files(2)
    indexer.insert(paths, collection_id="c6", timestamps=[10.0, 20.0])
    rows = indexer.db.open_table("c6").to_arrow().to_pylist()
    rows_by_path = {r["path"]: r for r in rows}
    assert rows_by_path[paths[0]]["timestamp"] == 10.0
    assert rows_by_path[paths[1]]["timestamp"] == 20.0


def test_insert_writes_vector_of_embedding_dim(indexer, mock_siglip_model, make_image_files):
    paths = make_image_files(2)
    indexer.insert(paths, collection_id="c7")
...
def test_insert_accepts_pil_images(indexer, fake_pil_image):
    ...
    job = indexer.insert(imgs, collection_id="c8", ids=["a", "b"])
    assert job.state == "done"
...
def test_insert_advances_processed_to_total(indexer, make_image_files):
    ...
    job = indexer.insert(paths, collection_id="c9")
...
def test_insert_raises_when_id_already_exists(indexer, make_image_files):
    paths = make_image_files(3)
    indexer.insert(paths, collection_id="c10")
...
def test_insert_failure_marks_job_failed(indexer, make_image_files):
    paths = make_image_files(2)
    indexer.insert(paths, collection_id="c11")
    with pytest.raises(ValueError):
        indexer.insert(paths, collection_id="c11", job_id="dead-job")
    ...
    with pytest.raises(RuntimeError, match="explode"):
        indexer.insert(paths2, collection_id="c11b", job_id="boom")
    s = get_status("boom")
...
def test_insert_into_empty_table_skips_conflict_check(indexer, make_image_files):
    ...
    paths = make_image_files(2)
    job = indexer.insert(paths, collection_id="fresh")
    assert job.state == "done"
...
def test_update_on_empty_table_inserts(indexer, make_image_files):
    paths = make_image_files(3)
    job = indexer.update(paths, collection_id="u1")
    assert job.state == "done"
...
def test_update_replaces_matching_rows(indexer, make_image_files):
    paths = make_image_files(3)
    indexer.insert(paths, collection_id="u2", timestamps=[1.0, 2.0, 3.0])
    # Re-embed the same paths with different timestamps; row count stays at 3,
    # but the timestamp columns reflect the new values.
    indexer.update(paths, collection_id="u2", timestamps=[10.0, 20.0, 30.0])
    table = indexer.db.open_table("u2")
    assert table.count_rows() == 3
    rows = {r["path"]: r["timestamp"] for r in table.to_arrow().to_pylist()}
    assert rows[paths[0]] == 10.0
    assert rows[paths[1]] == 20.0
    assert rows[paths[2]] == 30.0


def test_update_inserts_new_ids_alongside_existing(indexer, make_image_files):
    initial = make_image_files(2, subdir="a")
    indexer.insert(initial, collection_id="u3")
    extra = make_image_files(2, subdir="b")
    indexer.update(extra, collection_id="u3")
...
def test_update_does_not_raise_on_duplicate_ids_across_calls(indexer, make_image_files):
    paths = make_image_files(2)
    indexer.insert(paths, collection_id="u4")
    # Same paths → same ids; should upsert without complaining.
    job = indexer.update(paths, collection_id="u4")
    assert job.state == "done"
...
def test_update_failure_marks_job_failed_and_reraises(indexer, make_image_files):
    ...
    with pytest.raises(RuntimeError, match="upd-fail"):
        indexer.update(paths, collection_id="u5", job_id="updboom")
...
def test_insert_then_vector_search_returns_inserted_rows(indexer, make_image_files):
    ...
    indexer.insert(paths, collection_id="e2e")
...
def test_get_status_during_insert_reflects_progress(indexer, mock_siglip_model, make_image_files):
    ...
    indexer.insert(paths, collection_id="prog")
